...
describe('User Profile and Settings', () => {
  ...
  describe('Settings Dialog: Open and Close', () => {
    ...
    it('will not show the user avatar when not logged in', () => {
      const settingsDialog = bar.find(Dialog).filter('#user-settings-dialog');
      const avatarButtonInDialog = settingsDialog.find('#user-avatar').first();

      expect(avatarButtonInDialog.length).to.equal(0);
    });
  });
});
